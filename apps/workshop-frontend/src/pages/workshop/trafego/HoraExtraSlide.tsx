...
function Header() {
  return (
    <div className="text-center mb-4">
      ...
      <div className="text-black text-lg font-semibold">Tráfego – Hora Extra por Setor</div>
    </div>
  );
}

function Bar3D({ label, value, color, max = 8000 }: { label: string; value: number; color: string; max?: number }) {
...
export default function HoraExtraSlide() {
  const bars = [
    { label: 'Tráfego', value: 3420, color: '#0b3d91' },
    { label: 'Manutenção', value: 860, color: '#f59e0b' },
    { label: 'Administrativo', value: 420, color: '#ef4444' },
    { label: 'Jovem Aprendiz', value: 118, color: '#06b6d4' },
  ];
  ...
  return (
    <div className={`bg-white rounded-lg shadow ${pad}`}>
      ...
      <div className="relative bg-gradient-to-b from-emerald-50 to-emerald-200 rounded-md border border-emerald-300 p-6 overflow-hidden">
        {/* Meta */}
        <div className="absolute left-6 right-16" style={{ top: '75%' }}>
