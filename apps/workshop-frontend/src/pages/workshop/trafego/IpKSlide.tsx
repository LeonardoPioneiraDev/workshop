...
function Header() {
  return (
    <div className="text-center mb-4">
      ...
      <div className="text-black text-lg font-semibold">Tráfego – IPK por Setor</div>
    </div>
  );
}

function Bar3D({ label, value, color, max = 3 }: { label: string; value: number; color: string; max?: number }) {
...
export default function IpKSlide() {
  const bars = [
    { label: 'Tráfego', value: 1.92, color: '#0b3d91' },
    { label: 'Manutenção', value: 0.25, color: '#f59e0b' },
    { label: 'Administração', value: 0.12, color: '#ef4444' },
    { label: 'Jovem Aprendiz', value: 0.05, color: '#06b6d4' },
    { label: 'Total Operação', value: 1.85, color: '#0b3d91' },
  ];
