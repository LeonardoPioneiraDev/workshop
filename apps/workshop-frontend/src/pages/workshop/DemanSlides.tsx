...
function Bar3D({ label, value, color, max = 500000 }: { label: string; value: number; color: string; max?: number }) {
...
export default function DemanSlides() {
  const headers = ['Indicador', 'Descrição', 'Mês Atual'];
  const rows = [
    ['Diesel (L)', 'Consumo total no mês', 312345],
    ['Média (L/dia)', 'Consumo médio diário', 10411],
    ['Frota Abastecida', 'Qtde de veículos', 460],
  ];

  const bars = [
    { label: 'Diesel (L)', value: 312345, color: '#0b3d91', max: 500000 },
    { label: 'Média (L/dia)', value: 10411, color: '#f59e0b', max: 20000 },
    { label: 'Frota Abast.', value: 460, color: '#10b981', max: 1000 },
  ];

  return (
    <div className={`bg-white rounded-lg shadow ${pad}`}>
      <Header top="Pioneira" bottom="DEMAN - Abastecimento (Diesel)" />
