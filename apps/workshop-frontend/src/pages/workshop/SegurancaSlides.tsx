...
export default function SegurancaSlides() {
  const headers = ['Indicador', 'Descrição', 'Mês Atual'];
  const rows = [
    ['Colisões', 'Total de acidentes com colisão', 42],
    ['Sem vítimas', 'Colisões sem feridos', 31],
    ['Com vítimas', 'Colisões com feridos', 11],
  ];

  const bars = [
    { label: 'Colisões', value: 42, color: '#ef4444', max: 100 },
    { label: 'Sem vítimas', value: 31, color: '#10b981', max: 100 },
    { label: 'Com vítimas', value: 11, color: '#f59e0b', max: 100 },
  ];

  return (
    <div className={`bg-white rounded-lg shadow ${pad}`}>
      <Header top="Pioneira" bottom="Segurança - Acidentes (Colisões)" />
