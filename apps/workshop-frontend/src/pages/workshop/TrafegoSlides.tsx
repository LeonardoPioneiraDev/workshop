...
export default function TrafegoSlides() {
  const headers = ['Indicador', 'Descrição', 'Mês Atual'];
  const rows = [
    ['IPK', 'Passageiros por Km', 1.85],
    ['KM Ociosa', 'KM sem produção', 12500],
    ['Hora Extra', 'Horas excedentes', 3420],
  ];

  const bars = [
    { label: 'IPK', value: 1.85, color: '#0b3d91', max: 4 },
    { label: 'KM Ociosa', value: 12500, color: '#f59e0b', max: 20000 },
    { label: 'Hora Extra', value: 3420, color: '#ef4444', max: 8000 },
  ];

  return (
    <div className={`bg-white rounded-lg shadow ${pad}`}>
      <Header top="Pioneira" bottom="Tráfego - Indicadores" />
