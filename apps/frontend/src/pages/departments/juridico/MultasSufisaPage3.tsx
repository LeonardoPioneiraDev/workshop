  ...
  const fetchMultas = async (newFilters = {}) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Adicionar cidade mapeada a cada multa ANTES de aplicar filtros de cidade
        ...
        if (newFilters.mes && newFilters.mes !== 'Todos') {
          ...
          if (mesNumero) {
            multasSufisa = multasSufisa.filter(m => {
              const multaDate = new Date(m.dataHoraMulta);
              return !isNaN(multaDate.getTime()) && (multaDate.getMonth() + 1).toString().padStart(2, '0') === mesNumero;
            });
          }
        }
        
        if (newFilters.cidade && newFilters.cidade !== 'Todos') {
          multasSufisa = multasSufisa.filter(m => m.cidadeMapeada === newFilters.cidade);
        }
...
export default function MultasSufisaPage3() {
  ...
  const processedData = useMemo(() => {
    ...
    // ✅ 2. CLASSIFICAÇÃO - TODAS as classificações
    const classificacaoCount = {};
    multas.forEach(multa => {
      const classificacao = multa.grupoInfracao || 'Outras Infrações';
      classificacaoCount[classificacao] = (classificacaoCount[classificacao] || 0) + 1;
    });
    const dadosClassificacao = Object.entries(classificacaoCount)
      .map(([classificacao, quantidade]) => ({ 
        classificacao: classificacao.length > 18 ? classificacao.substring(0, 18) + '...' : classificacao,
        quantidade: Number(quantidade) || 0
      }))
      .sort((a, b) => b.quantidade - a.quantidade);

    // ✅ 3. DESMEMBRADO - Motivos detalhados (Top 10)
    const desmembradoCount = {};
    multas.forEach(multa => {
    ...
    multas.forEach(multa => {
      const linha = multa.linhaDescricao || 'Não informado';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-900/20 via-slate-900 to-yellow-900/20 text-white flex flex-col">
        ...
        <div className="container mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-12 mt-4 lg:mt-0">
              ...
              <div className="flex items-center gap-2 bg-blue-600/90 border border-blue-400/50 rounded-lg p-2 text-white shadow-lg">
              
                  <div className="text-sm font-bold text-blue-100 uppercase tracking-wider">TOTAL</div>
                  <div className="text-1xsm font-extrabold text-white">
      ...
      <div className="flex-1 flex flex-col mx-auto px-4 lg:px-5 py-6 space-y-6 w-full max-w-screen-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
          ...
          >
            <Card className="bg-gradient-to-br from-slate-800/80 to-amber-900/30 border-amber-500/30 backdrop-blur-sm flex flex-col h-full">
              <CardHeader className="flex-shrink-0 pb-4">
                <CardTitle className="text-white text-center">LINHA, VEÍCULO, TOTAL MULTAS</CardTitle>
              </CardHeader>
          ...
          >
            <Card className="bg-gradient-to-br from-slate-800/80 to-amber-900/30 border-amber-500/30 backdrop-blur-sm flex flex-col h-full">
              <CardHeader className="flex-shrink-0 pb-4">
                <CardTitle className="text-white text-center">TOTAL MULTAS, LOCAL</CardTitle>
              </CardHeader>
              <CardContent className="flex-1 overflow-y-auto">
                {loading ? (
                  <div className="flex justify-center items-center h-full max-h-[200px]">
